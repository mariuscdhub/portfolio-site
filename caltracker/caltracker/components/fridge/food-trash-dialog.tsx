...
import { getDeletedFoods, restoreFood, hardDeleteFood, emptyFoodTrash } from "@/lib/actions";
import { CustomFood } from "@/lib/actions";

export function FoodTrashDialog() {
    const [isOpen, setIsOpen] = useState(false);
    const queryClient = useQueryClient();

    const { data: deletedFoods = [], isLoading } = useQuery<CustomFood[]>({
        queryKey: ['foods', 'trash'],
        queryFn: () => getDeletedFoods() as Promise<CustomFood[]>,
        enabled: isOpen,
    });

    const restoreMutation = useMutation({
        mutationFn: restoreFood,
        onSuccess: () => {
    ...
    const hardDeleteMutation = useMutation({
        mutationFn: hardDeleteFood,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['foods', 'trash'] });
        },
    });

    const emptyTrashMutation = useMutation({
        mutationFn: emptyFoodTrash,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['foods', 'trash'] });
        },
    });
                        ...
                        <div className="mb-6 pr-10 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-neutral-400 mt-1">
                                    Les aliments sont supprimés définitivement après 30 jours.
                                </p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                            {isLoading ? (
                                ...
                                deletedFoods.map((food) => (
                                    <div key={food.id} className="glass-panel border-red-500/10 p-4 rounded-xl flex items-center justify-between group">
                                        <div>
                                            <h4 className="font-bold text-neutral-300 line-through">{food.name}</h4>
                                            <p className="text-xs text-red-400 mt-1">
                                                Supprimé définitivement dans {getDaysRemaining(food.deletedAt)} jours
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                onClick={() => hardDeleteMutation.mutate(food.id)}
